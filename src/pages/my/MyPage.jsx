...
export default function MyPage() {
  ...
  const { data: advice, isLoading: adviceLoading, error: adviceError, refetch: refetchAdvice } = useAdvice()
  ...
  return (
    <div className="my-wrap">
      <div className="d-flex align-items-center justify-content-between mb-3">
        ...
        <Button as={Link} to="/" variant="outline-primary">홈으로</Button>
      </div>
      ...
      <Card className="p-3 mb-3">
        <div className="d-flex align-items-center gap-3">
          <div className="my-avatar">
            ...
            <button className="my-avatar-add" onClick={onPickImage} title="사진 선택">+</button>
        ...
        {adviceLoading ? (
          ...
          <div className="d-flex align-items-center justify-content-between">
            ...
            <Button size="sm" variant="outline-secondary" onClick={() => {
              ...
              fetch('https://korean-advice-open-api.vercel.app/api/advice', { cache: 'no-store' })
                .then(r => r.json())
                .then(data => {
        ...
        <div className="my-graph">
          <svg viewBox="0 0 320 140" className="w-100">
            {moodSummary.entries.map(([mood, value], idx) => {
              ...
              const y = 120 - height;
              return (
                <g key={mood} className="bar-group" onClick={() => {
                  const ym = `${monthDate.getFullYear()}-${String(monthDate.getMonth()+1).padStart(2,'0')}`
                  navigate(`/?month=${ym}&mood=${mood}`)
                }}>
                  <rect x={x} y={y} width={40} height={height} rx={8} className={`bar bar-${mood}`} />
                  <text x={x + 20} y={130} textAnchor="middle" className="label">{mood}</text>
                  <text x={x + 20} y={y - 6} textAnchor="middle" className="value">{value}</text>
                </g>
              )
            })}
          </svg>
